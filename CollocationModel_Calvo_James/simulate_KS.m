...
if strcmp(options.irf, 'N')
    if strcmp(options.seed,'Y')
        rng(219);       % Set random number generator seed
    end
    DMshocks = randn(T+1,1);
elseif strcmp(options.irf, 'Y')
    DMshocks = zeros(T+1,1);
    DMshocks(2) = 1;
end
...
Pt          = zeros(T,1);   
Yt          = zeros(T,1);  
Pt(1)       = (1/eq.Y)*Mt(2);         % Initialize at eqm P under no agg. uncertainty 
Yt(1)       = Mt(2)/Pt(1);         % Initialize at eqm Y under no agg. uncertainty 
...
for iterY = 1:50;
    Yin         = Yt(t); 
    Pin         = Pt(t); 
    st          = gridmake(glob.pPgridf,glob.agridf,DMt(t+1),Yin);
    pi = Pin/Pt(t-1);
    ...
    v           = solve_valfuncKS(c, st ,param,glob,options,1);    % [st(:,1)*(1/pi) st(:,2:end)]
...
X = [ones(T-options.burn,1) log(Yt(options.burn:T-1)) log(DMt(options.burn+2:T+1))];
y = log(Yt(options.burn+1:T));
% X = [ones(T-options.burn,1) log(Yt(options.burn:T-1)) log(DMt(options.burn+1:T))];
% y = log(Yt(options.burn+1:T));
[b ,~,~,~,stats] = regress(y,X);
cKS = b;
R2     = stats(1);
