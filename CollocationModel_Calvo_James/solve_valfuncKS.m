...
pPdist          = [s(:,1); pPstar];   % distribution of real prices given state vector
...
if (nargin<=5)  
    ...
    vE = glob.Emat*glob.H*(param.lambda*glob.Phiprime*cK + (1-param.lambda)*glob.Phiprime*cC);    % (dprod(ind, Phiprime)*cK + dprod((1-ind), Phiprime)*cC);
...
if (nargout==2)
    jac = [ glob.Phi,                               zeros(glob.Ns),          -param.beta*glob.Phi;
          ...
          -glob.Emat*glob.H*param.lambda*glob.Phiprime,    -glob.Emat*glob.H*(1-param.lambda)*glob.Phiprime,   glob.Phi       ];  
...
v.ystar     = [menufun('output',s,s(:,1),param,glob,options);
                menufun('output',s,pPstar,param,glob,options)]; 
v.lstar     = [menufun('labour',s,s(:,1),param,glob,options);
    menufun('labour',s,pPstar,param,glob,options)];
