...
% Requires Control System Toolbox FOR DLYAP function
...
s=m(:).*sqrt(diag(lyap(A,diag(S.^2)))); % std.dev. of y (used to us dlyap from function control toolbox)
z=cell(M,1); y=nan(M,N);
for i=1:M
    if n(i)>1
        z{i}=linspace(-1,1,n(i))*s(i);
    else
        z{i}=0;
    end
